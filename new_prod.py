...
if not len(card_url_list):
    ...
    for card_url in card_url_list:
        try:
            ...
            result = browser.execute_script(f"return __YOULA_STATE__.entities.products[0]")
        ...
        try:
            ...
            if len(custom_fields):
                ...
                _url = session.query(Url).filter(Url.url == card_url).order_by(Url.date_added.desc()).first()
                _url.is_parsed = True
